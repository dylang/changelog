...
function generate(project, versionRequested, options) {
    ...
    if (project.match(/github.com/)) {
        ...
        if (repo && repo[1]) {
            ...
            return github.changelog(repo, versionRequested, options.token);
        }
    ...
    if (project.split('/').length === 2) {
        ...
        return github.changelog(project, versionRequested, options.token);
    }
